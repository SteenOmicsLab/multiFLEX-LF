...
### Download GUI
The multiFLEX-LF GUI and CLI executables for Windows 10 systems can be downloaded here: https://github.com/SteenOmicsLab/multiFLEX-LF/releases.
The python source code can be downloaded here: https://gitlab.com/SteenOmicsLab/multiflex-lf/-/releases

## Installation
No installation is needed. Just move the zip file to your location of choice and extract it. To start the program, double-click multiFLEX_LF_GUI.exe. The program might take a while to start.
...
This file contains the calculated raw scores for all peptides of all proteins. Additionally, the following rows are included:
...
- **Reference Sample Identifier (3):** Based on this, multiFLEX-LF determines the sample(s) to use as reference sample(s). This needs to match exactly the value in the "Group" column for all samples that should be taken as reference. If more than one sample is given (e.g., in case of a control group) multiFLEX-LF calculates the median intensity over all reference samples for each peptide. \[--reference/-r\]
...
- **Modification Cutoff (5):** The RM score cutoff used to classify peptides as differentially modified. Default: 0.5 \[--mod_cutoff/-mc\]
...
**Clustering Options:**
- **Hierarchical Clustering (10):** If checked, multiFLEX-LF computes a hierarchical clustering of the peptides based on their RM scores over the samples. The results of the clustering are visualized in an interactive dendrogram and heatmap. This is saved in a html file (suffix \*\*_mFQ-LF_RM_scores_clustered_heatmap.html), which can be opened in any common internet browser. \[--clustering/-c\]
- **Cosine Similarity Cutoff (11):** Cutoff score for the cosine similarity for missing RM scores imputation for the clustering. The missing values of a peptide are imputed based on the median of the RM scores of the closest/most similar peptides to that peptide. The cosine similarity is computed to find all peptides with a similarity above the given cosine similarity cutoff. Default: 0.98 \[--cosine_similarity/-cos\]
- **Deseq2 Normalization (12):** If checked, the RM scores are normalized after imputation employing DESeq2. The normalization is computed for each given sample group separately. Therefore, it has no effect on the results if sample groups contain less than 2 samples. The peptides are clustered based on the DESeq2 normalized RM scores. \[--deseq2_normalization/-dn\]
...
To run the multiFLEX-LF CLI, you can run the following command: (non-essential options in brackets, please remove them before execution!)\
...
You can run the following to show a help massage and information about the parameters:\
`multiFLEX_LF_CLI.exe -h`

For run time reasons, if the CLI will be applied many times consecutively, it is better to install the required packages and run the python script as follows (non-essential options in brackets, please remove them before execution!):\
...
You can also run the following to show a help massage and information about the parameters:\
